...
n_model =YOLO('./model/sh_modified.pt') 
...
def read_id(img):
  ...
  resz = cv2.resize(id_cropped, (416,416))

  # Check if the ID placement is found and return "No ID Placement found" otherwise
  if id_cropped.any() == 'No ID Placement found':
    return 'Try Again'
  torch.cuda.empty_cache()
  # ID Number extraction
  ID_NUMBER = read_image(resz)
