...
def extract_id_front_and_average_confidence(img):
    """
    ...
    cropped = img[380:500, 335:]
    results = ocr_front_model(cropped)
...
def process_two_sides(image_path1, image_path2):

    # Function to detect face in an image
    def has_face(image_path):
        faces = face_cascade.detectMultiScale(image_path, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
        return len(faces) > 0, image_path

    # Process both images
    has_face1, image1 = has_face(image_path1)
    has_face2, image2 = has_face(image_path2)

    # Assign images to variables based on whether they have a face or not
    image_with_face = image1 if has_face1 else image2 if has_face2 else None
    image_without_face = image2 if not has_face2 else image1 if not has_face1 else None
    card_front = crop_card_front (image_with_face)
    data_front = read_text_front(card_front)
    id_front, avg_confidence_front = extract_id_front_and_average_confidence(card_front)

    # back image
    card_back = crop_card_back(image_without_face)
